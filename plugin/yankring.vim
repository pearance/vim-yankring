...
function! s:YRShow(...)
    ...
    " List is shown in order of replacement assuming using previous yanks
    " let output = output . (show_registers == 1 ? 'Reg ' : 'Num ')."  Content\n"
    " XXX:
    " let output = \"--- YankRing ---\n"
    let output = (show_registers == 1 ? 'Reg ' : 'Num ')."  Yanks      [? = Options]\n"
    let output = output . "===   ========================\n"
...
function! s:YRWindowStatus(show_help)
    ...
    " Cleaner YR buffer XXX: VIMEZ MOD
    let msg = (s:yr_search==""?"":';SearchRegEx='.s:yr_search)
    ...
    if a:show_help == 1 && getline(1) !~ 'selection'
        ...
        let msg =
                    ...
                    \ '" ,,           : Quit / close the yankring window'."\n".
                    \ '" ?            : Remove help text'."\n".
...
function! s:YRWindowOpen(results)
    ...
    syn match yankringHeaders #^Num   Yanks      \[? = Options\]$#
    syn match yankringUnderline #^===   ========================$#
    ...
    hi link yankringHeaders operator
    hi link yankringUnderline type
    hi link yankringItemNumber type
    ...
    nnoremap <buffer> <silent> ,,            :call <SID>YRWindowAction ('q'   , 'n')<CR>
